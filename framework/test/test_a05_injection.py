...
from conftest import csrf_token, require_rest_login
...
def test_sql_looking_preference_is_stored_as_data(http, base_url, require_server, username, password):
    ...
    r2 = http.post(
        ...
        data={"preferenceKey": "secSqliHttp", "preferenceValue": sqli, "moquiSessionToken": tok or ""},
        headers={"X-CSRF-Token": tok} if tok else {},
        timeout=10,
        allow_redirects=False,
    )
    assert r2.status_code not in (401, 403), "setPreference unauthorized after login; not an injection proof"
