...
Logger logger = LoggerFactory.getLogger("org.moqui.impl.sendEmailWithAttachments")
...
try {
    ...
    if (emailTemplate == null) ec.message.addError(ec.resource.expand('No EmailTemplate record found for ID of [${emailTemplateId}]',''))
    ...
    // prepare the subject
    String subject =  ec.resource.expand((String) subject, "")
    //logger.info("Subject set to " + subjectText + ".")
    ...
    for (def emailAttachment in emailAttachmentList) {
        logger.info("Attaching content - ${emailAttachment}")

        DataSource dataSource = ec.resource.getLocationDataSource((String) emailAttachment)
        email.attach(dataSource, (String) emailAttachment.tokenize("/").last(), "")
    }
    ...
    logger.info("Error in sendEmailWithAttachments groovy", t)
    throw new BaseException("Error in sendEmailWithAttachments", t)
}
