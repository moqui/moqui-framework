...
class PersonEntityTester extends Specification {
    ...
    private void cleanAll() {
        ec.entity.find("ars.test.PersonToRate").deleteAll()
        ec.entity.find("ars.test.Person").deleteAll()
    }

    def "test person entity create"() {
        when:
            //create record in Person
            def result = this.ec.service.sync().name("ars.rockycube.EndpointServices.create#EntityData")
                .disableAuthz()
                .parameters([
                    entityName: "ars.test.Person",
                    data: [oscis: 1]
                ])
                .call()
        then:
    ...
    def "test person entity create PersonToRate"() {
        ...
        //create record
        ec.entity.makeValue("ars.test.Person").setAll(oscis: 1).create()

        //create record in PersonToRate
        def result = this.ec.service.sync().name("ars.rockycube.EndpointServices.create#EntityData")
                .disableAuthz()
                .parameters([
                        entityName: "ars.test.PersonToRate",
                        data: [oscis: 1]
                ])
                .call()
    ...
    def "test create PersonToRate with correct oscis two times"() {
        ...
        result = this.ec.service.sync().name("ars.rockycube.EndpointServices.create#EntityData")
                .disableAuthz()
                .parameters([
                        entityName: "ars.test.PersonToRate",
                        data: [oscis: 1]
    ...
    assert eh.getDefinition('ars.test.Person')
